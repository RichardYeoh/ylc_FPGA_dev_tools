	...
	begin
		...
		case(state)
			...
			begin
				...
				//if wr_fifo_err happen, go to end state immediately
				else if (wr_wfifo_err)
					begin
						state <= S_END;
					end				
				...
				//////if wr_fifo_err happen,write out all valid data then go to end state
				////else if (wr_wfifo_err)
				////begin
				////	state <= S_END;
				////end
